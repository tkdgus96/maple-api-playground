  ...
  return (
    <ItemWrapper>
      ...
      <div>
        ...
        {!!Number(item.item_total_option.str) && (
          <div>STR : +{item.item_total_option.str}</div>
        )}
        {!!Number(item.item_total_option.dex) && (
          <div>DEX : +{item.item_total_option.dex}</div>
        )}
        {!!Number(item.item_total_option.int) && (
          <div>INT : +{item.item_total_option.int}</div>
        )}
        {!!Number(item.item_total_option.luk) && (
          <div>LUK : +{item.item_total_option.luk}</div>
        )}
        {!!Number(item.item_total_option.max_hp) && (
          <div>최대 HP : +{item.item_total_option.max_hp}</div>
        )}
        {!!Number(item.item_total_option.max_hp_rate) && (
          <div>최대 HP : +{item.item_total_option.max_hp}</div>
        )}
        {!!Number(item.item_total_option.max_mp) && (
          <div>최대 MP : +{item.item_total_option.max_mp}</div>
        )}
        {!!Number(item.item_total_option.max_hp_rate) && (
          <div>최대 MP : +{item.item_total_option.max_mp}</div>
        )}
        {!!Number(item.item_total_option.attack_power) && (
          <div>공격력 : +{item.item_total_option.attack_power}</div>
        )}
        {!!Number(item.item_total_option.boss_damage) && (
          <div>
            보스 몬스터 공격 시 데미지 : +{item.item_total_option.boss_damage}%
          </div>
        )}
        {!!Number(item.item_total_option.ignore_monster_armor) && (
          <div>
            몬스터 방어율 무시 : +{item.item_total_option.ignore_monster_armor}%
          </div>
        )}
        {!!Number(item.item_total_option.damage) && (
          <div>데미지 : +{item.item_total_option.damage}</div>
        )}
        {!!Number(item.item_total_option.all_stat) && (
          <div>올스탯 : +{item.item_total_option.all_stat}%</div>
        )}
        {!!Number(item.item_total_option.speed) && (
          <div>이동속도 : +{item.item_total_option.speed}</div>
        )}
        {!!Number(item.item_total_option.jump) && (
          <div>점프력 : +{item.item_total_option.jump}</div>
        )}
        {!!Number(item.item_total_option.equipment_level_decrease) && (
          <div>
